...
import java.util.LinkedList;
 ...
 class HashTable {
	...
	int hashCode(String key) {
		...
		int l = key.length();
		for (int i = 0; i < l; i++) {
			code =  (((int) key.charAt(i)) + code) * 67 + (int) Math.pow(a, l * (l - i)) ;
		}
		return code;
	}
...
public class proj4 {
	...
	public static void main(String[] args) {
		...
		File dictionary = new File("files/dictionary.txt") ;//new File(stdin.next());
		System.out.print("\nText File: ");
		File textFile = new File("files/in2.txt"); //new File(stdin.next());
		System.out.print("\nOutput File: ");
		File outFile =  new File("files/out.txt");//new File(stdin.next());
		stdin.close();
		dict = null;
		text = null;
		out = null;
